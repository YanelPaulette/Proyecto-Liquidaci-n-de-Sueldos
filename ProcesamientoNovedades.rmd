---
title: "Procesamiento de Novedades de Sueldos"
subtitle: "Liquidacion de Sueldos"
...
Primero, seleccionamos las columnas  con las que vamos a trabajar: 
...
# duplico la columna Horario, para usarla con Licencias, 
n2<- n2 %>%
  mutate(Licencia=Horario)%>%
  mutate(Lic=0)


# Especifico las licencias
...
# De la columna Licencias, sacas todo aquello que no sea una licencia ( sacar todo lo que sea hs)

# creo una nueva variable. pero if_else solo toma valores logicos, por ende me sirve para filtrar luego 
...
# Uso la columna "Lic" para filtrar. Cuando es = 0 (no es licencia), y en esos casos que borre el # #contenido  de la variable "Licencia"

# opcion 1 

n5 %>%
mutate(Licencia= if_else(Lic==0, " ", Licencia)) # No funciona 

#opcion 2 

glimpse(n5)
n5$Lic<- as.logical(n5$Lic)
...
n2$Licencia<- gsub(":49", " ", n2$Licencia)
n2$Licencia<- gsub("06:25 A 12", " ", n2$Licencia)
n2$Licencia<- gsub("06:25 A 12", " ", n2$Licencia)
...
diferencia de hs nocturnas #ver lo del corte a las 6..
...
#######
#Para saber los que no son licencia: 
...
######
#quiero las licencias en columnas, salen mal porque esta mal desde el inicio, #arreglar
...
# esto esta mal, ya que falta acomodar las licencias, para limpiarlas bien,pero es provisorio
...
n3<-n2 %>%
  pivot_longer(cols = c(`hs50`:`Calent`), names_to = "concepto", values_to = "cantidad")%>%
